...
def pending_first_list_view(request):  # 初审待办列表
    """展示本行待初审申请并分页。"""  # 视图文档字符串
    maybe = _deny_if_not_qi(request)  # 校验审核员角色
    if maybe:  # 若返回重定向则直接返回
        return maybe  # 结束请求
    qs = scope_by_bank(Application.objects.select_related("card_product"), request.user)  # 过滤本行数据
    ...
    return render(request, "pending_list.html", {"page_obj": page_obj})  # 渲染模板
...
def pending_second_list_view(request):  # 复审待办列表
    """展示本行待复审申请分页列表。"""  # 视图文档字符串
    maybe = _deny_if_not_qi(request)  # 角色校验
    if maybe:  # 若失败
        return maybe  # 返回
    qs = scope_by_bank(Application.objects.select_related("card_product"), request.user)  # 本行
    qs = qs.filter(status=Application.ST_PENDING_SECOND)  # 待复审
    paginator = Paginator(qs.order_by("init_audit_time"), 10)  # 按初审时间排序分页
    page_obj = paginator.get_page(request.GET.get("page"))  # 当前页
    return render(request, "second_pending_list.html", {"page_obj": page_obj})  # 渲染
