...
from apply.views.multi_step_form import chinese_to_pinyin  # 导入统一的拼音转换函数
    ...
    if request.method == "POST":
        ...
        if form.is_valid():  # 执行所有字段的 clean_xxx 和表单级 clean()
            ...
            log_admin(
                ...
                message=f'员工提交申请，申请人:{app.applicant_name}，卡种:{app.card_product.product_name}',
                user_id=request.user.id,
                user_name=request.user.username,
                target_type='Application',
                target_id=app.pk,
                after_status=Application.ST_PENDING_FIRST,
                request=request,
            )
...
def return_edit_view(request, pk: int):  # 定义退回补充编辑视图
    ...
    if request.method == "POST":  # 处理保存提交
        ...
        if form.is_valid():  # 校验表单
            ...
            # 记录后台操作日志
            log_admin(
                action='RETURN_EDIT_SUBMIT',
                message=f'退回补充资料重新提交，申请人:{obj.applicant_name}',
                user_id=request.user.id,
                user_name=request.user.username,
                target_type='Application',
                target_id=obj.pk,
                before_status=Application.ST_RETURNED,
                after_status=Application.ST_PENDING_FIRST,
                request=request,
            )
...
def return_multi_step_view(request, pk: int):
    ...
    if request.method == "POST":
        ...
        page_data = {}
        for field in page_fields:
            # 姓名拼音字段由系统自动生成（使用统一的拼音转换函数，支持姓氏多音字和特殊字符）
            if field.field_type == 'name_pinyin':
...
def _save_return_application(request, form_data, product, application):
    ...
    log_admin(
        action='RETURN_EDIT_SUBMIT',
        message=f'退回补充资料重新提交，申请人:{application.applicant_name}',
        user_id=request.user.id,
        user_name=request.user.username,
        target_type='Application',
...
def application_detail_view(request, pk: int):  # 定义申请详情视图
    ...
    compare_has_mismatch = False  # 是否有不一致字段
    compare_mismatch_count = 0  # 不一致字段数量
    if application.status == Application.ST_ISSUED and application.issue_data:  # 已发卡且有快照时
        ...
        # 计算不一致字段数量
        compare_mismatch_count = sum(1 for r in compare_rows if r.get('mismatch'))
        compare_has_mismatch = compare_mismatch_count > 0
    return render(  # 渲染详情模板
        ...
        "application_detail.html",  # 模板名
        {
            ...
            "form_pages": form_pages,
            "form_data": form_data,
            "compare_has_mismatch": compare_has_mismatch,
            "compare_mismatch_count": compare_mismatch_count,
        },
    )
