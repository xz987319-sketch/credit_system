template = open('apply/templates/return_edit.html', encoding='utf-8').read()
import re
...
for pos, tag in tags:
    if '{% if' in tag or '{% elif' in tag:
        ...
        open_ifs -= 1
    if 'field_type' in tag or 'validation_rule' in tag or '{% endif %}' in tag:
        changes.append((open_ifs, tag[:70]))

print('Nesting trace:')
for level, tag in changes:
